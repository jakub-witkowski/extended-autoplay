...
void attack(char fname[], au a[], int* u)
{
    ...
    for (int i = 0; i < *u; i++)
    {
        ...
        if ((strcmp(a[i].affiliation, "E") == 0) && (a[i].training_time > 0))
            combat_ready = -1;
        ...
        if ((strcmp(a[i].affiliation, "E") == 0) && (a[i].remaining_movement < 1))
            combat_ready = -1;

        /* has the unit already made an attack in the current round? */
        if ((strcmp(a[i].affiliation, "E") == 0) && (a[i].attack_count < 1))
            combat_ready = -1;

        /* conditions for attack satisfied */
        if ((strcmp(a[i].affiliation, "E") == 0) && (a[i].remaining_movement >= 1) && (a[i].attack_count == 1))
            combat_ready = 1;
